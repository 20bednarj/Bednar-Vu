...
```{r}
favArtist1 <- get_artist_audio_features(artist= "Ms. Lauryn Hill")
favArtist2 <- get_artist_audio_features(artist= "Erykah Badu")
favArtist3 <- get_artist_audio_features(artist= "2Pac")
```


```{r}
favArtist4 <- get_artist_audio_features(artist= "Lil Wayne")
```

```{python}

# grouping the dataset as per artist name and counting the songs per artist
artist_length streamHistoryGus.groupby('artistName',as_index=False).sum()
artist_song_cnt = streamHistoryGus.groupby('artistName',as_index=False).agg({"trackName": "nunique"})
